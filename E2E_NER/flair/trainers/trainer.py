...
class ModelTrainer:
            ...
            for self.epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                for batch_no, batch in enumerate(batch_loader):
                    ...
                    for batch_step in batch_steps:
                        ...
                        # forward pass
                        loss_ner, loss_capu = self.model.forward_loss(batch_step)
                        loss = loss_ner + loss_capu
                        # Backward
                        if use_amp:
                    ...
                    seen_batches += 1
                    train_loss += loss_ner.item()
                    train_loss_capu += loss_capu.item()
                    ...
                    if batch_no % modulo == 0:
                        log.info(
                            ...
                            f"{train_loss / seen_batches:.8f}|{train_loss_capu / seen_batches:.8f} - samples/sec: {mini_batch_size * modulo / batch_time:.2f}"
                        )
                        batch_time = 0
                ...
                train_loss_capu /= seen_batches

                self.model.eval()

                log_line(log)
                log.info(
                    f"EPOCH {self.epoch} done: loss {train_loss:.4f}|{train_loss_capu:.4f} - lr {learning_rate:.4f}"
                )
                ...
                if log_train:
                    train_eval_result, train_loss, train_loss_capu = self.model.evaluate(
                        DataLoader(
                            self.corpus.train,
                ...
                if log_train_part:
                    train_part_eval_result, train_part_loss, train_part_loss_capu = self.model.evaluate(
                        DataLoader(
                            train_part,
                            batch_size=mini_batch_chunk_size,
                            num_workers=num_workers
                ...
                if log_dev:
                    dev_eval_result, dev_loss, dev_loss_capu = self.model.evaluate(
                        DataLoader(
                            self.corpus.dev,
                            batch_size=mini_batch_chunk_size,
                            num_workers=num_workers,
                    ...
                    log.info(
                        f"DEV : loss {dev_loss}|{dev_loss_capu}  - score {dev_eval_result.main_score}"
                    )
                ...
                if log_test:
                    test_eval_result, test_loss, test_loss_capu = self.model.evaluate(
                        DataLoader(
                            self.corpus.test,
                            batch_size=mini_batch_chunk_size,
                            num_workers=num_workers,
                    ...
                    log.info(
                        f"TEST : loss {test_loss}|{test_loss_capu} - score {test_eval_result.main_score}"
                    )
    ...
    def final_test(
        ...
        test_results, test_loss, test_loss_capu = self.model.evaluate(
            DataLoader(
                self.corpus.test,
        ...
        **kwargs,
        ...
        while step < iterations:
            ...
            for batch in batch_loader:
                ...
                # forward pass
                loss_ner, loss_capu = self.model.forward_loss(batch)
                loss = loss_ner + loss_capu
